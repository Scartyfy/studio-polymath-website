...
import { useDismiss } from "@/components/ui/expandable-action-bar-utils/use-dismiss";
import { useHoverGesture } from "@/components/ui/expandable-action-bar-utils/use-hover-gesture";
import { useTapGesture } from "@/components/ui/expandable-action-bar-utils/use-tap-gesture";
    ...
    <LayoutGroup id={layoutId}>
      ...
      >
        <motion.div
          ...
          layout="size"
          className={cn(
            "scrollbar-hide relative inline-flex max-w-full items-center overflow-x-auto overflow-y-hidden rounded-none border border-border bg-card/90 shadow-2xl backdrop-blur-xl",
            SIZE_CLASS[size],
            classNames?.track,
          )}
          transition={ITEM_TRANSITION}
          ...
          {items.map((item) => {
            ...
            return (
              <motion.button
                ...
                transition={ITEM_TRANSITION}
                className={cn(
                  "relative isolate inline-flex shrink-0 items-center justify-center overflow-hidden rounded-none font-medium text-muted-foreground outline-none transition-[color,background-color] duration-150 ease-out",
              ...
              >
                {isHighlighted ? (
                  <motion.span
                    ...
                    className="absolute inset-0 -z-10 rounded-none bg-primary/[0.07]"
                    transition={ITEM_TRANSITION}
                  />
                ) : null}
                ...
                {renderItem ? (
                  ...
                  <>
                    ...
                    {item.badge ? (
                      <span
                        className={cn(
                          "ml-0.5 inline-flex h-4 min-w-4 items-center justify-center rounded-none bg-destructive px-1 text-[10px] leading-none text-primary-foreground",
